...
export interface EngineProps extends React.HTMLAttributes<HTMLSpanElement> {
    enabled?: boolean,
    columnGap?: number
...
const TextEngine: NextPage<EngineProps> = memo(function Text({
    ...
    const words = useMemo<string[][]>(() => children.toString().split(' ').map((word: string) => word.split('')), [children])
    const letters = useMemo(() => words.flat(), [words])
    const [lines, _setLines] = useState<{ word: HTMLSpanElement, index: number, lineIndex: number }[][]>([])
    ...
    const wrapLineStyle = useCallback((wordIndex: number) => {
    ...
    const lineStyle = useCallback((wordIndex: number) => {
        const index = lines.flat().find(_ => _.index === wordIndex)?.lineIndex || 0
    ...
    const delayOutTimeoutOut = useRef<any>()
    useEffect(() => {
        if (inView && enabled) {
            // line
    ...
    return (
        <VarTextTag tag={tag} ref={ref} style={{ columnGap: `${columnGap}em`, display: 'flex', flexWrap: 'wrap' }} className={className} {...props}>
            {words.map((word: string[], wordIndex: number) => (
                <animated.span style={{ ...wrapLineStyle(wordIndex), ...{ overflow: overflow ? 'hidden' : 'initial', display: 'inline-block' } }} className={'line-word' + (wrapLineClassName ? ' ' + wrapLineClassName : '')} key={wordIndex}>
                    <animated.span style={{display: 'inline-block', ...lineStyle(wordIndex)}} className={lineClassName}>
                        <animated.span style={{ display: 'inline-block', ...wrapWordSprings[wordIndex], ...{ overflow: overflow ? 'hidden' : 'initial' } }} className={wrapWordClassName}>
                            <animated.span style={{display: 'inline-block', ...wordSprings[wordIndex]}} className={wordClassName}>
                                {word.map((letter: string, letterIndex: number) => {
                                    const index = words.slice(0, wordIndex).flat().length + letterIndex;
                                    return (
                                        <animated.span key={letterIndex} style={{ display: 'inline-block', ...wrapLetterSprings[index], ...{ overflow: overflow ? 'hidden' : 'initial' } }} className={wrapLetterClassName}>
                                            <animated.span style={{display: 'inline-block', ...letterSprings[index]}} className={letterClassName} >
                                                {letter}
                                            </animated.span>
                                        </animated.span>
                                    );
...
export const useResizeObserver = (trigger: MutableRefObject<any>, rerender: (newWidth: number, oldWidth: number) => void) => {
...
function calcLinesRefs(containerRef: MutableRefObject<any>) {
